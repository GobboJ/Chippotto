        ...
        match self.opcode & 0xF000 {
            0x000 => {
                match self.opcode & 0x00FF {
                    ...
                    _ => { println!("{} is not a valid opcode", self.opcode); self.pc += 2;}
                }
            },
            0x1000 => {
            ...
            0x8000 => {
                ...
                match self.opcode & 0x000F {
                    0x0000 => { self.reg[i]  = self.reg[j]; },
                    0x0001 => { self.reg[i] |= self.reg[j]; },
                    0x0002 => { self.reg[i] &= self.reg[j]; },
                    0x0003 => { self.reg[i] ^= self.reg[j]; },
                    0x0004 => {
                    ...
                    0x0007 => {
                        ...
                        self.reg[i] = self.reg[j] - self.reg[i];
                    },
                    0x000E => {
                        self.reg[i] = self.reg[j];
                        self.reg[i] <<= 1;
                        self.reg[15] = self.reg[j] & 0b1000_0000;
                    },
                    _ => { println!("{} is not a valid opcode", self.opcode); }
                }
                self.pc += 2;
            },
            0x9000 => {
                ...
                for yline in 0..h {
                    ...
                    for xline in 0..8 {
                        if (pixel & (0x80 >> xline)) != 0 {
                            ...
                            self.display[((x + xline + ((y+yline) * 64)) % (64 * 32)) as usize] ^= 1;
                        }
                    }    
                }
                self.pc += 2;
            ...
            0xF000 => {
                ...
                match self.opcode & 0x00FF {
                    ...
                    0x0029 => {
                        let i = ((self.opcode & 0x0F00) >> 8) as usize;
                        self.index = self.reg[i] as u16; // WRONG
                    },
                    0x0033 => {
                        let i = ((self.opcode & 0x0F00) >> 8) as usize;
                        self.memory[self.index as usize] = self.reg[i] / 100;
                        self.memory[self.index as usize + 1] = (self.reg[i] / 10) % 10;
                        self.memory[self.index as usize + 2] = (self.reg[i] % 100) % 10;
                    },
                    0x0055 => {
                        let i = ((self.opcode & 0x0F00) >> 8) as usize;
                        for v in 0..i+1 {
